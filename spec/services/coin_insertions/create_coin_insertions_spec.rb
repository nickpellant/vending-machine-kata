...
require 'services/coin_insertions/create_coin_insertions'

RSpec.describe Services::CoinInsertions::CreateCoinInsertions do
  ...
  let(:unvalidated_params) { { coin_id: coin.id, state: 'received' } }
  ...
  context 'when coin insertion data is valid' do
    let(:coin_insertion) { Factory.structs[:coin_insertion, :received, coin_id: coin.id] }
    ...
    before do
      ...
      allow(coin_insertion_repo).to(
        ...
        .and_return([coin_insertion, coin_insertion])
      )
    end
    ...
    it 'creates the coin insertion' do
      ...
      expect(coin_insertion_repo).to(
        have_received(:create).with([validated_params, validated_params])
      )
    end

    it 'returns the created coin insertions' do
      expect(call).to eql([coin_insertion, coin_insertion])
    end
  end
